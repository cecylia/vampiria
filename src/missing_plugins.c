...
static gboolean plugin_install_plugin_is_blacklisted(const gchar *detail)
{
	GList *res;
...
static void plugin_install_blacklist_plugin(const gchar *detail)
{
	if (!plugin_install_plugin_is_blacklisted(detail)) {
		blacklisted_plugins = g_list_prepend(blacklisted_plugins,
		                                     g_strdup(detail));
	}
}

static void plugin_install_context_free(PluginInstallContext *ctx)
{
...
static void plugin_install_done(PluginInstallContext *ctx, gboolean retry)
{
	GValue param[2] = { {0,}, {0,} };

	fprintf (stderr,
                 "invoking plugin install context %p callback: retry %d\n",
                 ctx,
                 retry);
...
static void on_plugin_installation_done(GstInstallPluginsReturn res,
                                        gpointer user_data)
{
	PluginInstallContext *ctx =(PluginInstallContext *) user_data;
	gchar **p;
	gboolean retry;
	...
	switch (res)
	{
		...
		case GST_INSTALL_PLUGINS_SUCCESS:
		{
			...
			for (p = ctx->details; p != NULL && *p != NULL; ++p) {
				plugin_install_blacklist_plugin(*p);
			}
		...
		{
			fprintf(stderr,
                                "Missing plugin installation failed: %s\n",
	...
	plugin_install_context_free(ctx);
}
                                 ...
                                 GClosure *closure)
{
	...
	ctx = g_new0(PluginInstallContext, 1);
	ctx->closure = g_closure_ref(closure);
	ctx->details = g_strdupv((char **)details);

	num = g_strv_length(ctx->details);
	for (i = 0; i < num; ++i) {
		if (ignore_blacklist == FALSE &&
                    plugin_install_plugin_is_blacklisted(ctx->details[i])) {
			fprintf(stderr,
                                "Missing plugin: %s(ignoring)\n",
                                ctx->details[i]);
	...
	if (parent_window != NULL &&
            gtk_widget_get_realized(GTK_WIDGET(parent_window))) {
#ifdef GDK_WINDOWING_X11
		gulong xid = 0;
	...
	status = gst_install_plugins_async((const gchar* const*) ctx->details,
                                            install_ctx,
	...
	if (status != GST_INSTALL_PLUGINS_STARTED_OK) {
		if (status == GST_INSTALL_PLUGINS_HELPER_MISSING) {
			fprintf(stderr,
                                "Automatic missing codec installation not supported "
                                "(helper script missing)\n");
		} else {
			fprintf(stderr,
                                "Failed to start codec installation: %s\n",
...
void missing_plugins_init(GtkWindow *window)
{
