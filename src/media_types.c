...
{
	if (!strcmp(mime_type, "application/x-id3") ||
            !strcmp(mime_type, "audio/mpeg"))
		return "audio/mpeg";
	else if (!strcmp(mime_type, "application/ogg") ||
                 !strcmp(mime_type, "audio/x-vorbis") ||
                 !strcmp(mime_type, "audio/ogg"))
		return "audio/x-vorbis";
	else if (!strcmp(mime_type, "audio/flac"))
		return "audio/x-flac";
	else if (!strcmp(mime_type, "audio/aac") ||
                 !strcmp(mime_type, "audio/mp4") ||
                 !strcmp(mime_type, "audio/m4a"))
		return "audio/x-aac";
...
gboolean
media_type_matches_profile(GstEncodingProfile *profile, const char *media_type)
{
	const GstCaps *pcaps;
	const GList *cl;
	GstCaps *caps;
	...
	caps = media_type_to_caps(media_type);
	if (caps == NULL)
		return FALSE;
	...
	if (GST_IS_ENCODING_CONTAINER_PROFILE(profile)) {
		...
		for (; cl != NULL; cl = cl->next) {
			...
			if (GST_IS_ENCODING_AUDIO_PROFILE(p)) {
				return caps_to_media_type(gst_encoding_profile_get_format(p));
			}
		...
		return caps_to_media_type(gst_encoding_profile_get_format(profile));
	}
}
